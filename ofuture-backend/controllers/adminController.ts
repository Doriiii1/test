...
// ═════════════════════════════════════════════
// E. USER STATUS MANAGEMENT
// ═════════════════════════════════════════════

const toggleUserStatus = async (req: AdminRequest, res: Response): Promise<any> => {
  const { id } = req.params;
  ...
  try {
    ...
    if (user.role === 'admin') {
      return res.status(403).json({
        success: false,
        message: 'Admin accounts cannot be locked/unlocked through the API.',
      });
    }
    ...
    const eventType = action === 'lock' ? LOG_EVENTS.USER_LOCKED : LOG_EVENTS.USER_UNLOCKED;
    ...
    await LogModel.write({
      ...
      eventType,
      severity: 'warn',
      message: `Admin ${action}ed user: ${user.username} (${user.id})`,
      payload: { targetId: id, action, reason, previousStatus: user.status, newStatus },
    });

    res.status(200).json({
      success: true,
      message: `User ${user.username} has been ${action}ed.`,
      data: { userId: id, action, newStatus },
    });
...
const updateWithdrawalStatus = async (req: AdminRequest, res: Response): Promise<any> => {
  const { id } = req.params;
  ...
  try {
    // Note: Assuming there's a withdrawals table. Adjust query as needed.
    const [[withdrawal]]: any = await pool.execute(
      'SELECT id, user_id, amount, status FROM withdrawals WHERE id = ? LIMIT 1',
      [id]
    );
    ...
    await pool.execute('UPDATE withdrawals SET status = ?, processed_at = NOW() WHERE id = ?', [newStatus, id]);

    await LogModel.write({
      ...ctx(req),
      eventType: 'WITHDRAWAL_' + action.toUpperCase(),
      severity: 'info',
      message: `Admin ${action}d withdrawal: $${withdrawal.amount} for user ${withdrawal.user_id}`,
      payload: { withdrawalId: id, action, reason, amount: withdrawal.amount },
    });

    res.status(200).json({
      success: true,
      message: `Withdrawal request has been ${action}d.`,
      data: { withdrawalId: id, action, newStatus },
    });

  } catch (err) {
    logger.error('updateWithdrawalStatus error:', err);
...
const resolveDispute = async (req: AdminRequest, res: Response): Promise<any> => {
  const { id } = req.params;
  const { decision, reason } = req.body; // decision: 'buyer' or 'seller'

  if (!['buyer', 'seller'].includes(decision)) {
    return res.status(400).json({
      success: false,
      message: 'Invalid decision. Use "buyer" or "seller".'
    });
  }

  try {
    // Note: Assuming there's a disputes table. Adjust query as needed.
    const [[dispute]]: any = await pool.execute(
      'SELECT id, order_id, buyer_id, seller_id, amount, status FROM disputes WHERE id = ? LIMIT 1',
      [id]
    );
    ...
    if (dispute.status !== 'pending') {
      return res.status(409).json({
        success: false,
        message: `Dispute is already resolved (${dispute.status}).`
      });
    }

    // Update dispute status
    await pool.execute('UPDATE disputes SET status = ?, resolved_by = ?, resolved_at = NOW(), decision = ? WHERE id = ?',
      ['resolved', req.user.id, decision, id]);

    // Handle escrow based on decision
    if (decision === 'buyer') {
      // Refund to buyer
      await pool.execute('UPDATE escrow_transactions SET status = ?, refunded_at = NOW(), refund_reason = ? WHERE order_id = ?',
        ['refunded', reason, dispute.order_id]);
    } else {
      // Release to seller
      await pool.execute('UPDATE escrow_transactions SET status = ?, released_at = NOW(), release_reason = ? WHERE order_id = ?',
        ['released', reason, dispute.order_id]);
    }
    ...
    await LogModel.write({
      ...
      severity: 'info',
      message: `Admin resolved dispute ${id}: funds to ${decision}`,
      payload: { disputeId: id, decision, reason, amount: dispute.amount },
    });

    res.status(200).json({
      success: true,
      message: `Dispute resolved. Funds awarded to ${decision}.`,
      data: { disputeId: id, decision, reason },
    });
...
const manageEscrow = async (req: AdminRequest, res: Response): Promise<any> => {
  const { id } = req.params;
  const { action, reason } = req.body; // action: 'release', 'refund', 'hold'

  if (!['release', 'refund', 'hold'].includes(action)) {
    return res.status(400).json({
      success: false,
      message: 'Invalid action. Use "release", "refund", or "hold".'
    });
  }

  try {
    const [[escrow]]: any = await pool.execute(
      'SELECT id, order_id, buyer_id, seller_id, amount, status FROM escrow_transactions WHERE id = ? LIMIT 1',
      [id]
    );
    ...
    if (!['held', 'disputed'].includes(escrow.status)) {
      return res.status(409).json({
        success: false,
        message: `Escrow is ${escrow.status}. Can only intervene on 'held' or 'disputed' transactions.`
      });
    }
    ...
    if (action === 'release') {
      newStatus = 'released';
      updateField = 'released_at';
      updateValue = 'NOW()';
    } else if (action === 'refund') {
      newStatus = 'refunded';
      updateField = 'refunded_at';
      updateValue = 'NOW()';
    } else {
      newStatus = 'held';
      updateField = 'held_at';
    ...
    await pool.execute(`UPDATE escrow_transactions SET status = ?, ${updateField} = ${updateValue}, admin_intervention = ? WHERE id = ?`,
      [newStatus, reason, id]);

    await LogModel.write({
      ...ctx(req),
      eventType: 'ESCROW_INTERVENTION',
      severity: 'warn',
      message: `Admin intervened in escrow ${id}: ${action}`,
      payload: { escrowId: id, action, reason, amount: escrow.amount },
    });

    res.status(200).json({
      success: true,
      message: `Escrow transaction has been ${action}d.`,
      data: { escrowId: id, action, newStatus },
    });

  } catch (err) {
    logger.error('manageEscrow error:', err);
    res.status(500).json({ success: false, message: 'Failed to manage escrow transaction.' });
  }
};
...
const getSystemLogs = async (req: AdminRequest, res: Response) => {
  try {
    const { page = '1', limit = '50', level, eventType, userId, from, to } = req.query;
    ...
    if (level) { conditions.push('severity = ?'); params.push(level); }
    ...
    const [rows]: any = await pool.execute(
      `SELECT
         id, event_type, severity, message, user_id, ip_address,
         user_agent, endpoint, method, payload, created_at
       FROM logs
       ${where}
       ORDER BY created_at DESC
       LIMIT ? OFFSET ?`,
